...
- TODO: figure out why point estimates are different for col 3 and 6
...
political_ols_data <- ols_data %>%
  left_join(rev_per_worker_12, by="prefect") %>%
  mutate(excess_protest_above_median = (d_event_pw > excess_event_median)) %>%
...
first_stage_f <- sapply(list(secretary_turnover_model, secretary_turnover_control_model,
                             secretary_turnover_control_het_model,
                             mayor_turnover_model, mayor_turnover_control_model,
                             mayor_turnover_control_het_model),
                        get_first_stage_f)
