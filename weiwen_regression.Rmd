...
- Q: Why are `lag_investment_pw` missing values replaced with 0?
...
- Q: Political turnover is a lagged effect too, why are we looking at `d_export_cus_pw` instead of `d2_event_pw`?
- Q: What's residual's median - 1 doing in the regression model?
- Q: Age bundary >60 or >=60?

- Note: two rows labelled ExportShock

```{r political_turnover_iv}
secretary_ols_data <- ols_data %>%
...
# TODO:
# reghdfe lead_turnover_s_d turnover_s_d d_event_pw_resid_g ///
# lead_tenure_incum i.lead_age_incum_g i.lead_prov_birth_incum_yesno i.lead_educ_incum i.lead_sex_incum  $controls ///
# (d_export_cus_pw c.d_event_pw_resid_g#c.d_export_cus_pw c.fiscal_rev_tot_pw12#c.d_export_cus_pw ///
# =d_export_btkrow_pw00 c.d_event_pw_resid_g#c.d_export_btkrow_pw00 c.fiscal_rev_tot_pw12#c.d_export_btkrow_pw00) if year>=2013 & year<=2015 [aweight=pop1564_2010], absorb(prefect province_year) keepsin cluster(province)
...
get_first_stage_f <- function(model) round(model$stage1$iv1fstat$d_export_cus_pw['F'])
first_stage_f <- sapply(list(secretary_turnover_model, secretary_turnover_control_model,
                             mayor_turnover_model, mayor_turnover_control_model),
                        get_first_stage_f)

stargazer(secretary_turnover_model, secretary_turnover_control_model,
          mayor_turnover_model, mayor_turnover_control_model,
          type='html',
          column.labels = rep(c('Party secretary', 'Mayor'), each=2),
          dep.var.labels.include = FALSE, omit.stat = c('adj.rsq', 'ser'),
          digits = 4, order=c(14, 1, 2, 3, 4, 5, 6, 7, 8),
          # omit = 'dln_', omit.labels = "Additional time-t controls?",
          add.lines = list(c('First-stage F-stat', first_stage_f)))
